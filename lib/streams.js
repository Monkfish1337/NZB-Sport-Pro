...
async function pipelineNativeNewznab({ promo, event, titles, indexers, torboxKey, urlCtx, log }) {
  ...
  const relevant = filterCandidates('native-newznab', discovered, log, promo, event);
  relevant.sort((a, b) => {
    const rb = resRank(b.title) - resRank(a.title);
    if (rb !== 0) return rb;
    const sb = (Number(b.size) || 0) - (Number(a.size) || 0);
  ...
  const [torboxRows, uuRows, easynewsRows, nativeNewznabRows] = await Promise.all([
    ...
    nativeNewznabEnabled
      ? runOrSkip('native-newznab', () => pipelineNativeNewznab({
          ...
          indexers: newznabIndexers, torboxKey, urlCtx, log,
        }))
      : Promise.resolve([]),
  ]);
