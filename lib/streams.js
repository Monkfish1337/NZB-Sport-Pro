...
function buildNativeNewznabRow(candidate, deferredUrl) {
  ...
  const ready = candidate.torboxCached === true;
  const metaLine = [
    sizeLabel ? '\u{1F4BE} ' + sizeLabel : '',
    candidate.indexer || 'Newznab',
    datePart,
    ready ? '\u26A1 TorBox cached - instant play' : '\u23F3 Uncached - click to queue',
  ].filter(Boolean).join(' · ');
  return {
    name: (ready ? '\u26A1 TorBox Usenet - Instant Play' : '\u23F3 TorBox Usenet - Queue')
      + '\n' + qualityLine,
    title: candidate.title + (metaLine ? '\n' + metaLine : ''),
    url: deferredUrl,
    behaviorHints: {
      bingeGroup: ready
        ? 'serioussportsync-native-newznab-ready' : 'serioussportsync-native-newznab-queue',
      notWebReady: false,
    },
  };
}
...
async function pipelineNativeNewznab({ promo, event, titles, indexers, torboxKey, urlCtx, log }) {
  ...
  const ordered = relevant.map((candidate, order) => ({
    ...
    const cached = Number(Boolean(b.prepared && b.prepared.cached))
      - Number(Boolean(a.prepared && a.prepared.cached));
    return cached || a.order - b.order;
  });
  const rows = [];
  for (const item of ordered.slice(0, MAX_ROWS)) {
    ...
    const candidate = Object.assign({}, item.candidate, {
      ...
      torboxCached: Boolean(preparedEntry && preparedEntry.cached),
  ...
  if (provider === 'torbox-usenet') {
    ...
    try {
      ...
      const outcome = await torboxUsenet.resolveNzb(
        ...
        {
          knownCached: candidate.torboxCached === true ? true : undefined,
          knownId: candidate.torboxOwnedId,
          nzbUrl: candidate.nzbUrl,
          useDirectLink: creds.nativeNewznabDirectLinkEnabled === true
            && candidate.torboxSharedMatchType === 'link',
        }
      );
