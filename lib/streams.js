...
//     Newznab search -> opaque signed row. On play-click only, SSS fetches the
//     selected NZB into bounded memory and uploads it to the user's TorBox
//     Usenet account. Stable pipelines remain independent and unchanged.
//
...
// short-lived memory store and is fetched only after this signed URL is hit.
...
function buildNativeNewznabRow(candidate, deferredUrl) {
  ...
  const metaLine = [
    ...
    datePart,
    'plays if ready; first click queues uncached NZB',
  ].filter(Boolean).join(' · ');
  return {
    name: '\u{1F9EA} TorBox Usenet\n' + qualityLine,
    title: candidate.title + (metaLine ? '\n' + metaLine : ''),
    url: deferredUrl,
    behaviorHints: { bingeGroup: 'serioussportsync-native-newznab', notWebReady: false },
  };
}
...
// Search is read-only. NZB retrieval and TorBox mutation are deferred until
// the signed stream URL is clicked.
async function pipelineNativeNewznab({ promo, event, titles, indexers, torboxKey, urlCtx, log }) {
  ...
  const rows = [];
  for (const candidate of relevant.slice(0, MAX_ROWS)) {
...
async function resolvePlay({ providerCode, eventId, infoHash, creds, username, userId }) {
  ...
  if (provider === 'torbox-usenet') {
    ...
    try {
      ...
      const nzbBuffer = await nativeNewznab.fetchNzb(candidate);
      const outcome = await torboxUsenet.resolveNzb(
        nzbBuffer, candidate.title || eventId, torboxKey, log
      );
      // Drop our only local reference immediately; the buffer is never stored.
