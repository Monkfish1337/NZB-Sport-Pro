...
async function prepareNativeNewznabCandidates(candidates, torboxKey, log) {
  ...
  const cachedHashes = await torboxUsenet.checkCachedMany(hashes, torboxKey, log, { timeoutMs: 3500 });
  for (const entry of prepared.values()) {
    entry.cached = (entry.hashes || [entry.hash]).some((hash) => cachedHashes.has(hash));
  }
