...
async function prepareNativeNewznabCandidates(candidates, torboxKey, log) {
    ...
    while (cursor < selected.length) {
      ...
      try {
        ...
        if (!entry) {
          ...
          entry = nativeNewznab.cachePreparedCandidate(candidate, buffer, hash)
            || { buffer, hash, key: '', expiresAt: 0 };
        }
  ...
  const hashes = Array.from(prepared.values()).map((entry) => entry.hash);
  const cachedHashes = await torboxUsenet.checkCachedMany(hashes, torboxKey, log, { timeoutMs: 3500 });
  for (const entry of prepared.values()) entry.cached = cachedHashes.has(entry.hash);
