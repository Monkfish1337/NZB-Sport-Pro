...
//
//   Pipeline D (experimental native Newznab): user's indexers -> event-aware
//     Newznab search -> bounded in-memory NZB preparation -> TorBox shared
//     cache check -> distinct instant-play / queue rows. Stable pipelines
//     remain independent and unchanged.
//
// Per-user requirements:
//   - At least one enabled configured pipeline on /account. Each pipeline has
//     its own switch so diagnosis never requires deleting credentials.
//
// Admin requirements:
...
const NATIVE_PREPARE_MAX = Math.max(1,
  Math.min(MAX_ROWS, parseInt(process.env.NATIVE_NEWZNAB_PREPARE_MAX || String(MAX_ROWS), 10)));
const NATIVE_PREPARE_CONCURRENCY = Math.max(1,
  Math.min(6, parseInt(process.env.NATIVE_NEWZNAB_PREPARE_CONCURRENCY || '4', 10)));
const NATIVE_CACHE_HASH_MAX = Math.max(100, Math.min(5000,
  parseInt(process.env.NATIVE_NEWZNAB_CACHE_HASH_MAX || String(torboxUsenet.CACHE_HASH_MAX), 10)));
  ...
  for (const r of noise.results) {
    ...
    if (verdict.ok) relevant.push(r);
      ...
      if (LOG_EXCLUDED) {
        ...
        // The shared release noise filter already logs its own
...
// Experimental native Newznab rows carry only an opaque candidate token.
// The credential-bearing NZB URL remains server-side in native-newznab's
// short-lived memory store. Top candidates may be prepared there for the
// shared-cache check; the client receives neither their URLs nor NZB bytes.
function buildNativeNewznabDeferredUrl({ origin, userId, apiToken, basePath, eventId, token }) {
  const { exp, sig } = urlSign.signResolve({
    userId, provider: 'torbox-usenet', eventId, infoHash: token,
  });
  const prefix = basePath || ('/u/' + encodeURIComponent(userId)
    + '/' + encodeURIComponent(apiToken));
  const path = prefix
    + '/resolve/torbox-usenet/' + encodeURIComponent(eventId)
    + '/' + encodeURIComponent(token)
    + '?exp=' + encodeURIComponent(exp) + '&sig=' + encodeURIComponent(sig);
  return (origin || '') + path;
}

function buildNativeNewznabRow(candidate, deferredUrl) {
  const resolution = detectResolution(candidate.title);
  const sourceTag = detectSource(candidate.title);
  ...
  const ready = candidate.torboxOwnedId != null;
  const processing = !ready && candidate.torboxPendingId != null;
  const sharedCached = !ready && !processing && candidate.torboxCached === true;
  const state = ready ? {
    icon: '\u26A1', name: 'TorBox Usenet - Instant Play', detail: 'Owned and ready - instant play', group: 'ready',
  } : processing ? {
    icon: '\u23F3', name: 'TorBox Usenet - Processing', detail: 'Already queued - click to wait', group: 'processing',
  } : sharedCached ? {
    icon: '\u{1F4E6}', name: 'TorBox Usenet - Cached', detail: 'TorBox cache match - click to attach and wait', group: 'cached',
  } : {
    icon: '\u23F3', name: 'TorBox Usenet - Queue & Wait', detail: 'Uncached - click to queue and wait', group: 'queue',
  };
  const metaLine = [
    sizeLabel ? '\u{1F4BE} ' + sizeLabel : '',
    candidate.indexer || 'Newznab',
    datePart,
    state.icon + ' ' + state.detail,
  ].filter(Boolean).join(' · ');
  return {
    ...
    url: deferredUrl,
    behaviorHints: {
      bingeGroup: 'nzb-sport-pro-native-newznab-' + state.group,
      notWebReady: false,
    },
  };
}
...
// Pipeline B - UU owns discovery; SSS owns event title generation/relevance.
async function pipelineUsenetUltimate({ promo, event, titles, uuConfig, log }) {
  if (!uuConfig) { log('uu: not configured - skipping pipeline B'); return []; }
  const searchOut = await uu.search(titles, uuConfig, {
    log,
    maxQueries: promo && promo.uuMaxQueries,
  });
  if (searchOut.results.length === 0) return [];
  const relevant = filterCandidates('uu', searchOut.results, log, promo, event);
  relevant.sort((a, b) => {
    const rb = resRank(b.title) - resRank(a.title);
    if (rb !== 0) return rb;
    const sb = (Number(b.size) || 0) - (Number(a.size) || 0);
...
async function prepareNativeNewznabCandidates(candidates, torboxKey, log) {
  const selected = (candidates || []).slice(0, NATIVE_PREPARE_MAX);
  ...
  async function worker() {
    while (cursor < selected.length) {
      ...
      try {
        let entry = nativeNewznab.getPreparedCandidate(candidate);
        if (!entry) {
          const buffer = await nativeNewznab.fetchNzb(candidate, { timeoutMs: 4500 });
          const hash = torboxUsenet.nzbHash(buffer);
          const hashes = torboxUsenet.nzbCacheHashes(buffer, candidate.nzbUrl);
          entry = nativeNewznab.cachePreparedCandidate(candidate, buffer, hash, hashes)
            || { buffer, hash, hashes, key: '', expiresAt: 0 };
        }
        prepared.set(candidate, Object.assign({}, entry, { cached: false }));
      } catch (err) {
        log('  native-newznab prepare ' + (candidate.indexer || 'indexer') + ': '
          + redact(err.message));
      }
    }
  }
  await Promise.all(Array.from({ length: Math.min(NATIVE_PREPARE_CONCURRENCY, selected.length) }, worker));
  // Interleave strategies across candidates so the safety cap cannot let a
  // many-file NZB starve later candidates. TorBox's POST endpoint accepts far
  // more than the GET endpoint's 100 hashes, allowing message IDs from every
  // file to materially improve cross-indexer cache matching.
  const entries = Array.from(prepared.values());
  const hashes = [];
  for (let strategy = 0; hashes.length < NATIVE_CACHE_HASH_MAX; strategy += 1) {
    let added = false;
    for (const entry of entries) {
      const hash = (entry.hashes || [entry.hash])[strategy];
      if (!hash || hashes.includes(hash)) continue;
      hashes.push(hash);
      added = true;
      if (hashes.length >= NATIVE_CACHE_HASH_MAX) break;
    }
    if (!added) break;
  }
  log('native-newznab cache: checking ' + hashes.length + ' TorBox hash(es)');
  const [cachedHashes, ownedHashes] = await Promise.all([
    torboxUsenet.checkCachedMany(hashes, torboxKey, log, {
      timeoutMs: 6000,
      maxHashes: NATIVE_CACHE_HASH_MAX,
    }),
    torboxUsenet.getOwnedReadyHashes(torboxKey, log, { timeoutMs: 3500 }),
  ]);
  for (const [candidate, entry] of prepared) {
    const entryHashes = entry.hashes || [entry.hash];
    const ownedHash = entryHashes.find((hash) => ownedHashes.has(hash));
    entry.sharedCached = entryHashes.some((hash) => cachedHashes.has(hash));
    entry.sharedMatchType = torboxUsenet.nzbLinkHashes(candidate.nzbUrl)
      .some((hash) => cachedHashes.has(hash)) ? 'link' : (entry.sharedCached ? 'content' : '');
    entry.ownedId = ownedHash ? ownedHashes.get(ownedHash) : null;
    const remembered = torboxUsenet.getKnownDownload(torboxKey, entry.hash);
    entry.pendingId = entry.ownedId == null && remembered ? remembered.id : null;
    entry.cached = Boolean(ownedHash) || entry.sharedCached;
  }
  const ready = Array.from(prepared.values()).filter((entry) => entry.cached).length;
  const owned = Array.from(prepared.values()).filter((entry) => entry.ownedId != null).length;
  const shared = Array.from(prepared.values()).filter((entry) => entry.sharedCached).length;
  const sharedLinks = Array.from(prepared.values())
    .filter((entry) => entry.sharedMatchType === 'link').length;
  log('native-newznab cache: ' + ready + ' ready / ' + (prepared.size - ready)
    + ' queue / ' + (selected.length - prepared.size) + ' unprepared'
    + ' (' + shared + ' shared: ' + sharedLinks + ' link / ' + (shared - sharedLinks)
    + ' content, ' + owned + ' already owned)');
  return prepared;
}

// Experimental Pipeline D — per-user Newznab discovery -> TorBox Usenet.
// Search and cache classification are read-only. NZBs are held only in a
// bounded, expiring memory cache. TorBox mutation remains click-only.
function filterByMaxResultSize(candidates, maxResultSizeGb, log) {
  const limitGb = Number(maxResultSizeGb) || 0;
  if (limitGb <= 0) return candidates;
  const limitBytes = limitGb * 1000 * 1000 * 1000;
  const kept = (candidates || []).filter((candidate) => {
    const bytes = Number(candidate && candidate.size) || 0;
    return bytes <= 0 || bytes <= limitBytes;
  });
  const removed = (candidates || []).length - kept.length;
  if (removed && typeof log === 'function') {
    log('native-newznab size filter: removed ' + removed + ' result(s) over ' + limitGb + ' GB');
  }
  return kept;
}

const OPTIONAL_PRE_SHOW_PATTERN = /(?:\bprelim(?:s|inary|inaries)?\b|\bpre[\s._-]*show\b|\bcountdown\b|\bkick[\s._-]*off[\s._-]*show\b|\bzero[\s._-]*hour\b)/i;

function filterExcludedPreShows(candidates, excludePreShows, log) {
  if (!excludePreShows) return candidates;
  const kept = (candidates || []).filter((candidate) =>
    !OPTIONAL_PRE_SHOW_PATTERN.test(String(candidate && candidate.title || '')));
  const removed = (candidates || []).length - kept.length;
  if (removed && typeof log === 'function') {
    log('native-newznab pre-show filter: removed ' + removed + ' prelim/pre-show result(s)');
  }
  return kept;
}

async function pipelineNativeNewznab({ promo, event, titles, indexers, torboxKey, maxResultSizeGb, excludePreShows, urlCtx, log }) {
  if (!config.experimentalNativeNewznab) return [];
  if (!torboxKey) { log('native-newznab: no user TorBox key — skipping'); return []; }
  if (!Array.isArray(indexers) || !indexers.length) {
    log('native-newznab: no user indexers — skipping');
    return [];
  }
  const discovered = await nativeNewznab.multiSearch(titles, indexers, {
    log,
    maxQueries: promo && promo.uuMaxQueries,
  });
  if (!discovered.length) return [];
  const relevant = filterExcludedPreShows(filterByMaxResultSize(
    filterCandidates('native-newznab', discovered, log, promo, event),
    maxResultSizeGb, log,
  ), excludePreShows, log);
  ...
  const prepared = await prepareNativeNewznabCandidates(relevant, torboxKey, log);
  const ordered = relevant.map((candidate, order) => ({
    candidate,
    order,
    prepared: prepared.get(candidate) || null,
  })).sort((a, b) => {
    const stateRank = (entry) => entry && entry.ownedId != null ? 3
      : (entry && entry.pendingId != null ? 2 : (entry && entry.sharedCached ? 1 : 0));
    return stateRank(b.prepared) - stateRank(a.prepared) || a.order - b.order;
  });
  const rows = [];
  for (const item of ordered.slice(0, MAX_ROWS)) {
    const preparedEntry = item.prepared;
    const candidate = Object.assign({}, item.candidate, {
      nzbHash: preparedEntry ? preparedEntry.hash : '',
      preparedKey: preparedEntry ? preparedEntry.key : '',
      torboxCached: Boolean(preparedEntry && preparedEntry.sharedCached),
      torboxOwnedId: preparedEntry ? preparedEntry.ownedId : null,
      torboxPendingId: preparedEntry ? preparedEntry.pendingId : null,
      torboxSharedMatchType: preparedEntry ? preparedEntry.sharedMatchType : '',
    });
    const token = nativeNewznab.storeCandidate(urlCtx.userId, event.id, candidate);
    rows.push(buildNativeNewznabRow(candidate, buildNativeNewznabDeferredUrl({
      origin: urlCtx.origin,
      userId: urlCtx.userId,
      apiToken: urlCtx.apiToken,
      basePath: urlCtx.basePath,
      eventId: event.id,
      token,
    })));
  }
  log('native-newznab: built ' + rows.length + ' row(s)');
  return rows;
}
...
async function handleStream(params) {
  ...
  const torrentTitles = (typeof promo.torrentSearchTitles === 'function')
    ...
    : titles;
  const nativeNewznabTitles = (typeof promo.nativeNewznabSearchTitles === 'function')
    ? Array.from(new Set([].concat(promo.nativeNewznabSearchTitles(event) || [], event.searchAliases || [])
      .filter(Boolean)))
    : titles;
  ...
  // Undefined means enabled so existing users retain pre-toggle behaviour.
  const torboxEnabled = userConfig.torboxEnabled !== false;
  const uuEnabled = userConfig.uuEnabled !== false;
  const easynewsEnabled = userConfig.easynewsEnabled !== false;
  const nativeNewznabEnabled = config.experimentalNativeNewznab
    && userConfig.nativeNewznabEnabled === true;
  const newznabIndexers = nativeNewznabEnabled && Array.isArray(userConfig.newznabIndexers)
    ? userConfig.newznabIndexers : [];
  ...
  const [torboxRows, uuRows, easynewsRows, nativeNewznabRows] = await Promise.all([
    torboxEnabled
      ? runOrSkip('torbox', () => pipelineTorrentTorbox({ promo, event, titles: torrentTitles, torboxKey, urlCtx, log }))
      : Promise.resolve([]),
    uuEnabled
    ...
    easynewsEnabled
      ...
      : Promise.resolve([]),
    nativeNewznabEnabled
      ? runOrSkip('native-newznab', () => pipelineNativeNewznab({
          promo, event, titles: nativeNewznabTitles,
          indexers: newznabIndexers, torboxKey,
          maxResultSizeGb: userConfig.maxResultSizeGb,
          excludePreShows: userConfig.excludePreShows === true, urlCtx, log,
        }))
      : Promise.resolve([]),
  ]);

  if (!torboxEnabled) log('pipeline torbox disabled by user');
  if (!uuEnabled) log('pipeline uu disabled by user');
  if (!easynewsEnabled) log('pipeline easynews disabled by user');
  if (config.experimentalNativeNewznab && !nativeNewznabEnabled) {
    log('pipeline native-newznab disabled by user');
  }
  ...
  // Keep experimental rows separate from stable cross-provider dedupe so a
  // user can compare/diagnose native Newznab while UU or Easynews is enabled.
  for (const row of nativeNewznabRows) merged.push(row);
  const configuredMax = parseInt(String(userConfig.maxStreams || '0'), 10);
  const finalRows = Number.isFinite(configuredMax) && configuredMax > 0
    ? merged.slice(0, Math.min(20, configuredMax)) : merged;
  log('returning ' + finalRows.length + ' stream row(s) total');
  return { streams: finalRows };
}
...
// is the ONLY place the metadata addon ever mutates a user's playback account
// or requests a CDN link — /stream remains read-only.
//
...
async function resolvePlay({ providerCode, eventId, infoHash, creds, username, userId }) {
  ...
  const errorLog = (msg) => console.error(tag + ' ' + msg);
  const provider = String(providerCode || '').toLowerCase();

  if (provider === 'easynews') {
    if (creds && creds.easynewsEnabled === false) {
      return { ok: false, error: 'easynews-disabled' };
    }
    return resolveEasynews({ eventId, token: infoHash, creds, log });
  }
  if (provider === 'torbox-usenet') {
    if (!config.experimentalNativeNewznab) return { ok: false, error: 'experimental-disabled' };
    if (!creds || creds.nativeNewznabEnabled !== true) {
      return { ok: false, error: 'native-newznab-disabled' };
    }
    const torboxKey = String(creds.torboxApiKey || '').trim();
    if (!torboxKey) return { ok: false, error: 'no-torbox-key' };
    const candidate = nativeNewznab.getCandidate(infoHash, userId, eventId);
    if (!candidate) return { ok: false, error: 'candidate-expired' };
    if (!nativeNewznab.candidateStillConfigured(candidate, creds.newznabIndexers)) {
      return { ok: false, error: 'indexer-no-longer-configured' };
    }
    try {
      let nzbBuffer = nativeNewznab.getPreparedNzb(infoHash, userId, eventId);
      if (nzbBuffer) log('using prepared NZB from bounded memory cache');
      else {
        log('fetching selected NZB from ' + (candidate.indexer || 'user indexer'));
        nzbBuffer = await nativeNewznab.fetchNzb(candidate);
      }
      const outcome = await torboxUsenet.resolveNzb(
        nzbBuffer, candidate.title || eventId, torboxKey, log,
        {
          knownCached: candidate.torboxCached === true,
          knownId: candidate.torboxOwnedId != null
            ? candidate.torboxOwnedId : candidate.torboxPendingId,
          knownReady: candidate.torboxOwnedId != null,
          nzbUrl: candidate.nzbUrl,
          errorLog,
        }
      );
      // The local variable is released after this call. Prepared bytes remain
      // only in the bounded, expiring in-memory cache; they never reach disk.
      if (outcome.url) log('native Newznab resolved through TorBox Usenet');
      else if (outcome.processing) log('native Newznab still processing in user TorBox account');
      else if (outcome.queued) log('native Newznab queued in user TorBox account');
      return outcome;
    } catch (err) {
      log('native Newznab resolve failed: ' + redact(err.message));
      return { ok: false, error: 'native-newznab-resolve-failed' };
    }
  }
  if (provider !== 'torbox') {
  ...
  if (creds && creds.torboxEnabled === false) {
    log('torbox torrent pipeline disabled by user');
    return { ok: false, error: 'torbox-disabled' };
  }
  if (!torboxKey) {
...
// Candidate search for an explicit per-event admin power-tool request.
//
...
async function searchCandidates(event, log) {
  ...
  const companionCfg = settings.getCompanion();
  if (!companionCfg.url) {
    log('  searchCandidates: companion not configured — request skipped');
    return [];
  }
  try {
    const out = await companion.scrape({
      promotion: promo, event, searchTitles: torrentTitles, log,
    });
...
module.exports = {
  handleStream, resolvePlay, searchCandidates,
  _test: { filterByMaxResultSize, filterExcludedPreShows, OPTIONAL_PRE_SHOW_PATTERN },
};
