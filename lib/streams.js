...
async function handleStream(params) {
  ...
  // Pipeline A — torrent indexers (Prowlarr + Zilean in parallel) emit
  // infoHash rows. Prowlarr scrapes whatever Torznab feeds the admin has
  // configured. Zilean scrapes the public DMM hashlist — every hash there
  // is by definition already cached on a debrid somewhere, so it's
  // valuable for debrid users even if the hash isn't on a live tracker.
  // Results are merged + deduped by infoHash before filtering.
  tasks.push((async () => {
    try {
      log('torznab: querying ' + titles.length + ' title variant(s)');
      const [prowResults, zileanResults] = await Promise.all([
        prowlarr.multiSearch(titles, { log }).catch((err) => {
          log('  prowlarr pipeline error: ' + err.message); return [];
        }),
        zilean.multiSearch(titles, { log }).catch((err) => {
          log('  zilean pipeline error: ' + err.message); return [];
        }),
      ]);
      // Merge with dedup by infoHash. Prowlarr first (has seeder data),
      // Zilean fills in unique hashes Prowlarr didn't find.
      const seenHash = new Set();
      const merged = [];
      for (const r of prowResults) {
        if (!r.infoHash || seenHash.has(r.infoHash)) continue;
        seenHash.add(r.infoHash); merged.push(r);
      }
      for (const r of zileanResults) {
        if (!r.infoHash || seenHash.has(r.infoHash)) continue;
        seenHash.add(r.infoHash); merged.push(r);
      }
      log('torznab: merged ' + merged.length + ' unique hash(es) ('
        + prowResults.length + ' prowlarr + ' + zileanResults.length + ' zilean)');
      if (merged.length === 0) return [];
      const relevant = filterResults('torznab', merged, log, promo, event);
