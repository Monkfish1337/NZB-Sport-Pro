...
async function pipelineNativeNewznab({ promo, event, titles, indexers, torboxKey, maxResultSizeGb, urlCtx, log }) {
  ...
  const relevant = filterByMaxResultSize(
  ...
  const [torboxRows, uuRows, easynewsRows, nativeNewznabRows] = await Promise.all([
    ...
    nativeNewznabEnabled
      ? runOrSkip('native-newznab', () => pipelineNativeNewznab({
          ...
          maxResultSizeGb: userConfig.maxResultSizeGb, urlCtx, log,
        }))
      : Promise.resolve([]),
  ]);
...
module.exports = { handleStream, resolvePlay, searchCandidates, _test: { filterByMaxResultSize } };
