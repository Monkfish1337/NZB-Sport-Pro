...
async function prepareNativeNewznabCandidates(candidates, torboxKey, log) {
  ...
  // Interleave strategies across candidates so the 100-hash TorBox batch cap
  // cannot let a many-file NZB starve later candidates of a cache check.
  const entries = Array.from(prepared.values());
  const hashes = [];
  for (let strategy = 0; hashes.length < 100; strategy += 1) {
    let added = false;
  ...
  const [cachedHashes, ownedHashes] = await Promise.all([
    torboxUsenet.checkCachedMany(hashes, torboxKey, log, { timeoutMs: 3500 }),
