...
async function handleStream(params) {
  ...
  for (const r of noise.results) {
    ...
    if (verdict.ok) {
      ...
      log('  rel-filter drop (' + verdict.reason + '): ' + r.title);
    }
  }
  // Per-reason tally — useful for diagnosing "why empty" at a glance.
  const rejBreakdown = Object.entries(rejectReasons)
    .map(([k, v]) => v + ' ' + k).join(' / ') || 'none';
  log('relevance filter kept ' + relevant.length + ' / rejected: ' + rejBreakdown);

  // Top-line one-liner summary across all stages (easy log-grepping target).
  log('SUMMARY: ' + searchOut.results.length + ' raw -> '
    + noise.results.length + ' post-noise -> '
    + relevant.length + ' post-relevance');
