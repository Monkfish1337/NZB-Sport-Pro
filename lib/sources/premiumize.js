...
async function cacheCheck(hashes, apiKey, log) {
  ...
  for (let i = 0; i < all.length; i += BATCH) {
    ...
    // 0.25.1: defensive parse. j.response SHOULD be a boolean array parallel
    // to the items[] we sent. Only record an explicit verdict per index — if
    // PM returns a shorter array, the missing items stay 'unknown' (out.set
    // never called) so the row builder falls through to optimistic display.
    let cachedN = 0, notN = 0, unknownN = 0;
    for (let k = 0; k < slice.length; k++) {
      if (k < j.response.length && typeof j.response[k] === 'boolean') {
        out.set(slice[k], j.response[k]);
        if (j.response[k]) cachedN++; else notN++;
      } else {
        unknownN++;
      }
    }
    if (unknownN === slice.length && slice.length > 1) {
      log && log('    premiumize.cacheCheck: all ' + slice.length + ' items absent from response — '
        + 'check ADMIN_PM_KEY');
    } else if (unknownN > 0) {
      log && log('    premiumize.cacheCheck: ' + cachedN + ' cached, ' + notN + ' not, ' + unknownN + ' unknown');
    }
  }
  return out;
}
