...
async function checkCached(hash, apiKey, log, options) {
  log = log || (() => {});
  if (!/^[a-f0-9]{32}$/i.test(String(hash || '')) || !apiKey) return false;
  const params = new URLSearchParams({ hash: String(hash).toLowerCase(), format: 'object' });
  const url = TORBOX_API_BASE + '/usenet/checkcached?' + params.toString();
  try {
    const response = await ((options && options.fetchImpl) || fetch)(url, httpAgent.fetchOpts({
      headers: authHeaders(apiKey), timeout: 10000,
    }, url));
    if (!response.ok) { log('  torbox-usenet: checkcached HTTP ' + response.status); return false; }
    return cachedFromPayload(await response.json(), String(hash).toLowerCase());
  } catch (err) {
    log('  torbox-usenet: checkcached ' + redact(err.message));
    return false;
  }
}

function multipartNzb(buffer, filename) {
...
async function resolveNzb(buffer, title, apiKey, log, options) {
  ...
  const cached = await checkCached(hash, apiKey, log, options);
  let id = cacheGet(apiKey, hash);
  if (id == null) {
    id = await createDownload(buffer, title, apiKey, log, options);
    if (id != null) cacheSet(apiKey, hash, id);
  }
  if (id == null) return { ok: false, error: 'create-failed' };
  const url = await findPlayable(id, apiKey, log, Object.assign({}, options, {
