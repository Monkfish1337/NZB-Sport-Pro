...
function multipartNzbLink(link, fields) {
  const boundary = '----sss-' + crypto.randomBytes(16).toString('hex');
  const chunks = [];
  const values = Object.assign({ link }, fields || {});
...
async function createDownload(buffer, title, apiKey, log, options) {
  ...
  const opts = options || {};
  const directLink = opts.useDirectLink === true ? String(opts.nzbUrl || '') : '';
  const upload = directLink
    ? multipartNzbLink(directLink, {
        name: title || 'SeriousSportSync',
        add_only_if_cached: opts.cachedOnly ? 'true' : undefined,
      })
    : multipartNzb(buffer, (title || 'serioussportsync') + '.nzb', {
        add_only_if_cached: opts.cachedOnly ? 'true' : undefined,
      });
  ...
  if (opts.cachedOnly) {
    if (directLink) log('  torbox-usenet: attaching cached NZB by direct indexer link');
    else log('  torbox-usenet: attaching cached NZB (' + Math.ceil(upload.body.length / 1024) + ' KiB)');
  }
  let response;
  try {
...
function ownedReadyHashesFromPayload(payload) {
  ...
  for (const job of jobs) {
    ...
    if (!ready || job.download_present === false) continue;
...
async function resolveNzb(buffer, title, apiKey, log, options) {
  ...
  const cached = knownId != null ? true : (typeof opts.knownCached === 'boolean'
    ? opts.knownCached
    : (await checkCachedMany(cacheHashes, apiKey, log, opts)).size > 0);
  let id = knownId != null ? knownId : cacheGet(apiKey, hash);
  if (id == null) {
  ...
  if (id == null && cached) {
    ...
    for (let attempt = 0; attempt < attempts && id == null; attempt += 1) {
      ...
      const owned = await getOwnedReadyHashes(apiKey, log, Object.assign({}, opts, {
        forceRefresh: true,
