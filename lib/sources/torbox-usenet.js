...
function nzbCacheHashes(buffer, links) {
  if (!Buffer.isBuffer(buffer) || !buffer.length) return [];
  const hashes = [];
  // Message IDs come first because TorBox documents them as its best Usenet
  // matching strategy.
  for (const id of firstMessageIds(buffer)) hashes.push(md5Text(id));
  ...
  for (const rawLink of (Array.isArray(links) ? links : [links])) {
    ...
    if (!link) continue;
    hashes.push(md5Text(link));
    const normalized = normalizedUsenetLink(link);
    if (normalized) hashes.push(md5Text(normalized));
  }
...
async function getOwnedReadyHashes(apiKey, log, options) {
  ...
  if (cached && cached.expiresAt > Date.now()) return new Map(cached.hashes);
  const params = new URLSearchParams({ limit: '1000' });
  const url = TORBOX_API_BASE + '/usenet/mylist?' + params.toString();
  try {
    const response = await ((options && options.fetchImpl) || fetch)(url, httpAgent.fetchOpts({
      headers: authHeaders(apiKey), timeout: Number((options && options.timeoutMs) || 6000),
    }, url));
    if (!response.ok) {
...
async function resolveNzb(buffer, title, apiKey, log, options) {
  ...
  const cached = knownId != null ? true : (typeof opts.knownCached === 'boolean'
    ? opts.knownCached
    : (await checkCachedMany(nzbCacheHashes(buffer, opts.nzbUrl), apiKey, log, opts)).size > 0);
