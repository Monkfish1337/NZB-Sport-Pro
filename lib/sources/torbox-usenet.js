...
// endpoint is far more restrictive than the main API's 300/min — even an
// 8s backoff between retries isn't always enough to clear. Mitigations:
//   - maxQueries=1: only fire the SHORTEST title variant (Newznab's `q`
//     is fuzzy text-search, so a single broad query like "UFC 291" already
//     covers what 3 alias variants would have).
//   - rateLimitBackoffMs=15000: give the limiter 15s on a 429 before
//     retrying once.
//   - queryDelayMs irrelevant when maxQueries=1 but kept for safety if
//     the cap is ever raised.
// The in-memory result cache (newsnab.js, 1h TTL) means repeat clicks on
// the same event skip the API entirely.
