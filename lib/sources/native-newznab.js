...
const PREPARED_NZB_TTL_MS = Math.max(5 * 60 * 1000,
  parseInt(process.env.NATIVE_NEWZNAB_PREPARED_TTL_MINUTES || '30', 10) * 60 * 1000);
const PREPARED_NZB_MAX_BYTES = Math.max(8 * 1024 * 1024,
  parseInt(process.env.NATIVE_NEWZNAB_PREPARED_MAX_BYTES || String(64 * 1024 * 1024), 10));
...
const ALLOW_PROXY = /^(1|true|yes|on)$/i.test(String(process.env.NATIVE_NEWZNAB_ALLOW_PROXY || ''));
...
const preparedNzbs = new Map(); // URL fingerprint -> { buffer, hash, expiresAt }
let preparedNzbBytes = 0;
...
function privateIp(address) {
  ...
  if (kind === 4) return privateIpv4(raw);
  if (kind !== 6) return true;
  const value = raw.toLowerCase();
  if (value === '::1' || value === '::' || value.startsWith('fc') || value.startsWith('fd')
      || /^fe[89ab]/.test(value) || value.startsWith('ff') || value.startsWith('fec')
      || value.startsWith('100:') || value.startsWith('2001:db8:')
      || value.startsWith('2001:0:') || value.startsWith('2002:')) return true;
  ...
  if (mappedHex) {
    ...
    return privateIpv4([(high >> 8) & 255, high & 255, (low >> 8) & 255, low & 255].join('.'));
  }
  // Public indexers should resolve to global unicast IPv6 (2000::/3). A
  // conservative allow rule avoids obscure transition/reserved ranges being
  // used to reach infrastructure the application did not intend to expose.
...
// Indexers commonly canonicalise their public hostname by adding or removing
// "www". Treat only that exact spelling variant as the same credential owner;
// redirects to sibling subdomains or unrelated hosts remain blocked.
function sameProviderHost(left, right) {
  const canonical = (value) => String(value || '').trim().toLowerCase()
    .replace(/\.$/, '').replace(/^www\./, '');
  const a = String(left || '').trim().toLowerCase().replace(/\.$/, '');
  const b = String(right || '').trim().toLowerCase().replace(/\.$/, '');
  return Boolean(a && b && (a === b || (canonical(a) === canonical(b)
    && (a.startsWith('www.') || b.startsWith('www.')))));
}

async function resolveHost(urlValue) {
  const host = new URL(urlValue).hostname.replace(/^\[|\]$/g, '');
  if (net.isIP(host)) {
    if (!ALLOW_PRIVATE && privateIp(host)) throw new Error('private-address-blocked');
    return [{ address: host, family: net.isIP(host) }];
  }
  const resolved = await dns.lookup(host, { all: true, verbatim: true });
  if (!resolved.length || (!ALLOW_PRIVATE && resolved.some((entry) => privateIp(entry.address)))) {
    throw new Error('private-address-blocked');
  }
  return resolved;
}

function pinnedAgent(urlValue, resolved) {
  const target = new URL(urlValue);
  const entries = (resolved || []).map((entry) => ({
    address: entry.address,
    family: Number(entry.family) || net.isIP(entry.address),
  }));
  if (!entries.length) throw new Error('host-resolution-empty');
  let cursor = 0;
  const lookup = (_hostname, options, callback) => {
    const all = options && typeof options === 'object' && options.all === true;
    if (all) return callback(null, entries.slice());
    const entry = entries[cursor++ % entries.length];
    return callback(null, entry.address, entry.family);
  };
  return target.protocol === 'http:' ? new http.Agent({ lookup }) : new https.Agent({ lookup });
}
...
function safeResultLink(raw, endpoint) {
  ...
  if (link.protocol !== base.protocol || !sameProviderHost(link.hostname, base.hostname)) return '';
...
async function safeFetch(url, options) {
  ...
  for (let redirect = 0; redirect <= 2; redirect += 1) {
    const resolved = typeof opts.resolveImpl === 'function'
      ? await opts.resolveImpl(current)
      : (opts.fetchImpl && ALLOW_PRIVATE
          ? [{ address: '127.0.0.1', family: 4 }]
          : await resolveHost(current));
    const directOptions = {
      headers: opts.headers || { Accept: 'application/rss+xml, application/xml, text/xml' },
      timeout: Number(opts.timeoutMs || 7000),
      redirect: 'manual',
      agent: pinnedAgent(current, resolved),
    };
    // A conventional HTTP proxy resolves the CONNECT target itself, defeating
    // DNS pinning. Keep user-controlled Newznab traffic direct by default;
    // operators may explicitly accept that trust trade-off for a controlled
    // outbound proxy with NATIVE_NEWZNAB_ALLOW_PROXY=on.
    const requestOptions = ALLOW_PROXY
      ? httpAgent.fetchOpts(directOptions, current)
      : directOptions;
    const response = await (opts.fetchImpl || fetch)(current, requestOptions);
    if (![301, 302, 303, 307, 308].includes(response.status)) return response;
    const location = response.headers && response.headers.get('location');
    if (!location) throw new Error('redirect-without-location');
    const next = new URL(location, current);
    if (!sameProviderHost(next.hostname, baseHost) || next.protocol !== new URL(url).protocol) {
...
function buildConnectionTestUrl(indexer) {
  const url = new URL(indexer.url);
  url.searchParams.set('t', 'search');
  url.searchParams.set('q', 'NZB-Sport-Pro connection test');
  url.searchParams.set('apikey', indexer.apiKey);
  url.searchParams.set('limit', '1');
  return url.toString();
}

async function testConnection(indexer, options) {
  let clean;
  try { clean = normalizeIndexerConfig(indexer, 0); }
  catch (err) { return { ok: false, status: 0, error: 'invalid-config' }; }
  try {
    const response = await safeFetch(buildConnectionTestUrl(clean), Object.assign({}, options, {
      headers: { Accept: 'application/xml, text/xml, application/rss+xml' },
      timeoutMs: Number(options && options.timeoutMs || 7000),
    }));
    if (!response.ok) {
      return {
        ok: false,
        ...
        error: response.status === 401 || response.status === 403
          ? 'invalid-key' : (response.status === 429 ? 'rate-limited' : 'http-error'),
      };
    }
    const body = (await readLimited(response, 512 * 1024)).toString('utf8');
    if (/<error\b/i.test(body)) return { ok: false, status: response.status, error: 'provider-error' };
    if (!/<rss\b/i.test(body)) return { ok: false, status: response.status, error: 'unexpected-response' };
    return { ok: true, status: response.status };
  } catch (err) {
    const detail = redact(err && err.message || 'network-error');
    let error = 'network-error';
    if (/timed?\s*out|timeout/i.test(detail)) error = 'timeout';
    else if (/private|local|redirect|dns|address/i.test(detail)) error = 'blocked-address';
    else if (/response-too-large/i.test(detail)) error = 'unexpected-response';
...
async function searchOne(indexer, query, options) {
  ...
  log('  native-newznab ' + indexer.name + ': query "' + String(query).replace(/[\r\n]+/g, ' ') + '"');
  try {
...
function preparedKey(candidate) {
  if (!candidate || !candidate.nzbUrl) return '';
  return crypto.createHash('sha256').update([
    candidate.indexerUrl || '',
    candidate.indexerKeyFingerprint || '',
    candidate.nzbUrl,
  ].join('|')).digest('hex');
}

function deletePrepared(key) {
  const entry = preparedNzbs.get(key);
  if (entry && Buffer.isBuffer(entry.buffer)) {
    preparedNzbBytes = Math.max(0, preparedNzbBytes - entry.buffer.length);
  }
  preparedNzbs.delete(key);
}

function prunePrepared(incomingBytes) {
  const now = Date.now();
  for (const [key, entry] of preparedNzbs) {
    if (entry.expiresAt <= now) deletePrepared(key);
  }
  const needed = Math.max(0, Number(incomingBytes) || 0);
  while (needed && preparedNzbBytes + needed > PREPARED_NZB_MAX_BYTES) {
    const oldest = preparedNzbs.keys().next().value;
    if (!oldest) break;
    deletePrepared(oldest);
  }
}

function cachePreparedCandidate(candidate, buffer, hash, hashes) {
  if (!Buffer.isBuffer(buffer) || !buffer.length || buffer.length > PREPARED_NZB_MAX_BYTES) return null;
  const key = preparedKey(candidate);
  if (!key) return null;
  deletePrepared(key);
  prunePrepared(buffer.length);
  if (preparedNzbBytes + buffer.length > PREPARED_NZB_MAX_BYTES) return null;
  const entry = {
    key,
    buffer,
    hash: String(hash || crypto.createHash('md5').update(buffer).digest('hex')).toLowerCase(),
    hashes: Array.from(new Set((hashes || [hash])
      .map((value) => String(value || '').toLowerCase())
      .filter((value) => /^[a-f0-9]{32}$/.test(value)))),
    expiresAt: Date.now() + PREPARED_NZB_TTL_MS,
  };
  preparedNzbs.set(key, entry);
  preparedNzbBytes += buffer.length;
  return Object.assign({}, entry);
}

function getPreparedCandidate(candidate) {
  prunePrepared(0);
  const key = String(candidate && candidate.preparedKey || '') || preparedKey(candidate);
  const entry = key && preparedNzbs.get(key);
...
function storeCandidate(userId, eventId, result, preparedNzb) {
  const prepared = Buffer.isBuffer(preparedNzb)
    ? cachePreparedCandidate(result, preparedNzb, result && result.nzbHash) : getPreparedCandidate(result);
  pruneCandidates();
  const token = crypto.randomBytes(20).toString('hex');
  candidates.set(token, {
    userId: String(userId || ''),
    eventId: String(eventId || ''),
    result: Object.assign({}, result, prepared ? { preparedKey: prepared.key } : {}),
...
function getPreparedNzb(token, userId, eventId) {
  const key = String(token || '').toLowerCase();
  const entry = candidates.get(key);
  if (!entry || entry.userId !== String(userId || '') || entry.eventId !== String(eventId || '')) return null;
  if (entry.expiresAt <= Date.now()) { candidates.delete(key); return null; }
  const prepared = getPreparedCandidate(entry.result);
  return prepared && Buffer.isBuffer(prepared.buffer) ? prepared.buffer : null;
}
...
module.exports = {
  ...
  parseNewznabXml,
  multiSearch,
  cachePreparedCandidate,
  getPreparedCandidate,
  storeCandidate,
  getCandidate,
  getPreparedNzb,
  fetchNzb,
  _test: {
    candidates, decodeXml, privateIp, sameProviderHost, safeResultLink,
    readLimited, resolveHost, pinnedAgent,
  },
};
