...
async function hydrateAll(results, log, concurrency) {
  ...
  // 0.32.1: configurable cap so heavier indexer setups can recover more
  // results at the cost of latency. Each hydration is one HTTP roundtrip
  // through Prowlarr's download proxy, so the practical ceiling is around
  // 100 before /stream feels sluggish.
  const HYDRATE_MAX = parseInt(process.env.PROWLARR_HYDRATE_MAX || '50', 10);
  const needs = results
