...
//   Body: { promotion, event, searchTitles, requestId }
...
async function scrape({ promotion, event, searchTitles, log }) {
  log = log || (() => {});
  ...
  const timeoutMs = parseInt(process.env.COMPANION_TIMEOUT_MS || '30000', 10);
  let res;
  try {
  ...
  catch (err) { log('  companion: bad JSON: ' + err.message); return []; }
