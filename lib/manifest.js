...
function buildManifest(opts) {
  ...
  const selectedCatalogs = (selected.length > 0)
    ? allCatalogs.filter((c) => selected.includes(c.id))
    : allCatalogs;
  // Keep the catalog resource advertised even when home rows are hidden:
  // imported Nuvio collections call the catalog endpoints by addon/catalog ID.
  // Only the manifest's visible catalog descriptors are suppressed.
  const catalogs = (userCfg && userCfg.showCatalogsOnHome === false)
    ? []
    : selectedCatalogs;
