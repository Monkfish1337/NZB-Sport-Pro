...
async function createUser({ username, password, role }) {
  ...
  const user = {
    ...
    lastSeen: null,
    // 0.31.1: uuManifestUrl is the only active backend field. TorBox direct
    // integration was retired (see lib/sources/torbox-usenet.js). Debrid
    // keys remain in the schema for backward compat with existing JSON but
    // unused by /stream. Cleanup pass will drop rd/tb/pm/autoCache later.
    config: {
      uuManifestUrl: '',
      rd: '', tb: '', pm: '',
      catalogs: [],
