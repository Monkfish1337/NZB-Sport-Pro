...
async function createUser({ username, password, role }) {
  ...
  const user = {
    ...
    lastSeen: null,
    // 0.31.0: uuManifestUrl and torboxApiKey are the active fields. Either
    // (or both) configures a Usenet backend for /stream. Debrid keys remain
    // in the schema for backward compat with existing JSON but unused by
    // /stream. Cleanup pass will drop rd/tb/pm/autoCache in a follow-up.
    config: {
      uuManifestUrl: '',
      torboxApiKey: '',
      rd: '', tb: '', pm: '',
