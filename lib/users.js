...
//     createdAt, lastSeen, config: { rd, tb, pm, catalogs, showCatalogsOnHome, promotionOrder, catalogOrder }
//   }
...
//
// 0.34.0: torboxApiKey + easynewsPassword join the encryption list. The
...
function encryptUserKeysInPlace(user) {
  ...
  if (Array.isArray(user.config.newznabIndexers)) {
    user.config.newznabIndexers = user.config.newznabIndexers.map((item) => {
      const copy = Object.assign({}, item || {});
      if (copy.apiKey) copy.apiKey = cryptoKeys.encrypt(copy.apiKey);
      return copy;
    });
  }
}
...
function decryptedUser(user) {
  ...
  if (Array.isArray(cfg.newznabIndexers)) {
    cloneCfg.newznabIndexers = cfg.newznabIndexers.map((item) => {
      const copy = Object.assign({}, item || {});
      if (copy.apiKey) copy.apiKey = cryptoKeys.decrypt(copy.apiKey);
      return copy;
    });
  }
...
async function createUser({ username, password, role }) {
  ...
  const user = {
    ...
    lastSeen: null,
    // 0.33.0: two active backend fields.
    //   - uuManifestUrl: per-user UU instance (URL handoff, unchanged).
    //   - torboxApiKey:  per-user TorBox key used by the metadata addon
    //                    to resolve hashes returned by the companion
    //                    scraper into playable URLs. Per-user isolation
    //                    preserved; key never leaves this addon.
    // 0.34.0: Easynews credentials drive Pipeline C (direct Easynews
    ...
    config: {
      ...
      easynewsUsername: '',
      easynewsPassword: '',
      // Pipeline switches default on for backward compatibility. Undefined
      // also means on when older users.json records are loaded.
      torboxEnabled: true,
      uuEnabled: true,
      easynewsEnabled: true,
      // Experimental-only fields remain inert unless the server flag is on.
      nativeNewznabEnabled: false,
      nativeNewznabDirectLinkEnabled: false,
      newznabIndexers: [],
      rd: '', tb: '', pm: '',
      catalogs: [],
      ...
      promotionOrder: [],
      catalogOrder: [],
      autoCache: { rd: false, tb: false, pm: false },
      maxStreams: 0,
    },
  };
