...
function buildAccessToken(id) {
  return ACCESS_PREFIX + '.' + id + '.' + accessSecret(id);
}
...
function create(input) {
  ...
  saveAll(state);
  return {
    accessToken: buildAccessToken(id),
...
function resolveAccess(token) {
  ...
  if (!parsed || !safeEqual(parsed.secret, accessSecret(parsed.id))) return null;
...
function resolveEdit(token) {
  ...
  try {
    return {
      ...
      accessToken: buildAccessToken(found.record.id),
    };
  } catch (_) {
    return null;
  }
...
module.exports = {
  create, resolveAccess, resolveEdit, update,
