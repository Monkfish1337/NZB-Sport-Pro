...
const TABLER_CSS_INTEGRITY = 'sha384-fkzLEr+R1vlsMqNHsxTOSmtcmf0+LuY6VkpET9w9/SVpXLQQXzUsSuF91EOMmk+n';
const TABLER_JS_INTEGRITY = 'sha384-NLYyWBhfSOpSirqEjYzqtCV1kzwtsSN0O77Jz/9XwdkuWvQPT3XfWTFz/maXL9eF';
...
const ADMIN_SECTIONS = [
  { id: 'admin',        label: 'Configurations', href: '/admin',
  ...
  { id: 'metadata-sync', label: 'Metadata sync', href: '/admin/metadata-sync',
    icon: 'M20 7h-9 m3 -3l-3 3l3 3 M4 17h9 m-3 -3l3 3l-3 3' },
...
function buildSidebar(currentSection) {
  ...
  return ''
    ...
    +         '<span class="sss-brand-logo me-2">N</span>'
    +         '<span class="text-white fw-semibold">NZB-Sport-Pro</span>'
    +       '</a>'
    +     '</div>'
...
function buildTopbar(user, isAdminLayout) {
  ...
  const userMenu = user ? ''
    ...
    +     '<a href="' + (role === 'admin' ? '/admin' : '/account') + '" class="dropdown-item">' + (role === 'admin' ? 'Administration' : 'Account') + '</a>'
...
:root, [data-bs-theme=dark] {
  --tblr-primary: #ff7a18;
  --tblr-primary-rgb: 255, 122, 24;
  --tblr-primary-fg: #fff;
  --tblr-primary-darken: #e86508;
  --tblr-primary-lt: rgba(255, 122, 24, 0.12);
  --tblr-link-color: #ff8e38;
  --tblr-link-hover-color: #ffad67;
...
.btn-primary, .btn-primary:hover, .btn-primary:focus, .btn-primary:active {
  --tblr-btn-bg: #ff7a18;
  --tblr-btn-border-color: #ff7a18;
  --tblr-btn-hover-bg: #e86508;
  --tblr-btn-hover-border-color: #e86508;
  --tblr-btn-active-bg: #c95605;
  --tblr-btn-active-border-color: #c95605;
}
.sss-brand-logo {
  display: inline-flex; align-items: center; justify-content: center;
  width: 32px; height: 32px;
  background: linear-gradient(145deg,#ff9d2e,#ff7a18); color: #111;
  border-radius: 6px;
...
function tablerPage(title, bodyHtml, opts) {
  ...
  return [
    ...
    '<link href="' + TABLER_CSS + '" rel="stylesheet" integrity="' + TABLER_CSS_INTEGRITY + '" crossorigin="anonymous">',
    ...
    bodyHtml,
    '</div></div></div></div>',
    '<script src="' + TABLER_JS + '" integrity="' + TABLER_JS_INTEGRITY + '" crossorigin="anonymous"></script>',
    '</body></html>',
  ].join('');
}
...
function authShell(title, bodyHtml) {
  return [
    ...
    '<span class="sss-brand-logo me-2" style="width:40px;height:40px;font-size:14px;">N</span>',
    '<span class="text-white fw-semibold fs-3">NZB-Sport-Pro</span>',
    '</a>',
    '</div>',
