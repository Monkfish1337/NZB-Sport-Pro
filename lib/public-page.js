...
function configurationPage({ config = {}, promotions = [], token = '' } = {}) {
  ...
  <main class="config-wrap">
    ...
    <section class="config-card">
      ...
      <div class="output${token ? ' show' : ''}" id="output"><label>Your private manifest URL</label><div class="output-row"><input class="input" id="manifest-url" readonly><button class="small-btn" id="copy-output" type="button">Copy</button></div><label style="margin-top:12px">Private editing link — store this separately</label><div class="output-row"><input class="input" id="edit-url" readonly><button class="small-btn" id="copy-edit" type="button">Copy</button></div><div style="margin-top:10px"><a id="collection-link" class="fine" href="#">Download Nuvio collection JSON</a></div><div class="config-management"><button class="small-btn" id="rotate-manifest" type="button">Rotate manifest</button><button class="small-btn danger-action" id="delete-config" type="button">Delete configuration</button></div></div>
  ...
  (function(){
    ...
    function applyConfig(config){document.getElementById('torbox-key').value=config.torboxApiKey||'';setIndexerRows(config.newznabIndexers||[]);var selected=new Set(config.catalogs||[]),all=selected.size===0;document.querySelectorAll('.catalog').forEach(function(input){input.checked=all||selected.has(input.value);});document.getElementById('show-home').checked=config.showCatalogsOnHome!==false;document.getElementById('max-streams').value=config.maxStreams||0;document.getElementById('max-result-size').value=config.maxResultSizeGb||0;document.getElementById('edit-state').innerHTML='<span class="edit-pill">Editing existing configuration</span>';}
