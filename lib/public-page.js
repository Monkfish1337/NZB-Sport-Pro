...
function configurationPage({ config = {}, promotions = [], token = '' } = {}) {
  ...
  (function(){
    ...
    function applyConfig(config){document.getElementById('torbox-key').value=config.torboxApiKey||'';setIndexerRows(config.newznabIndexers||[]);var selected=new Set(config.catalogs||[]),all=selected.size===0;document.querySelectorAll('.catalog').forEach(function(input){input.checked=all||selected.has(input.value);});document.getElementById('show-home').checked=config.showCatalogsOnHome!==false;document.getElementById('max-streams').value=config.maxStreams||0;document.getElementById('edit-state').innerHTML='<span class="edit-pill">Editing existing configuration</span>';}
