...
const f1 = {
  ...
  searchTitles(event) {
    ...
    // Race: "Formula 1 2026 Monaco GP" and "Formula 1 2026 Monaco GP Race"
    if (session === 'race') {
      if (year) {
...
// ===== MotoGP =====
// TheSportsDB league 4407. Like F1, each round is split across multiple
// sessions: Practice 1/2, Qualifying, Sprint Race (Saturday), and the main
// Race (Sunday). Scene rips are typically per-session, e.g.
//   MotoGP.2026.Round.06.SpanishGP.Race.MULTi.1080p.WEB-DL
//   MotoGP.2026.06.SpanishGP.Qualifying.MULTi
//   MotoGP.2026.SpanishGP.Sprint.WEB
...
function motogpSession(name) {
  ...
  if (/\bsprint\b/.test(n)) return 'sprint';
  if (/qualifying|qualification|\bquali\b/.test(n)) return 'qualifying';
  if (/practice|free[\s.\-_]*practice|\bfp[1-3]\b|\bwarm[\s.\-_]*up\b/.test(n)) return 'practice';
  return 'race';
}
...
const MOTOGP_SESSION_LABEL = {
  race: 'Race', qualifying: 'Qualifying', sprint: 'Sprint', practice: 'Practice',
};
...
const motogp = {
  ...
  posterShape: 'landscape',
  // TheSportsDB ships per-session thumbs for MotoGP weekends; prefer them.
  preferThumb: true,

  defaults: {
    poster: brandedPoster('motogp-upcoming.jpg', 'https://r2.thesportsdb.com/images/media/league/banner/4o72ed1740575810.png'),
    fanart: 'https://r2.thesportsdb.com/images/media/league/fanart/72b7nl1721988672.jpg',
    logo:   'https://r2.thesportsdb.com/images/media/league/logo/n4nfqj1733763283.png',
  },
  ...
  buildAliases(name) {
    ...
    out.add('MotoGP ' + t);
    if (loc) {
      out.add(('MotoGP ' + loc + ' GP ' + after).trim());
      out.add(('MotoGP ' + loc + ' Grand Prix ' + after).trim());
      out.add((loc.replace(/\s+/g, '') + 'GP ' + after).trim());
    }
    return Array.from(out).filter(Boolean);
  },

  // 0.30.0: short Newsnab queries. MotoGP scene naming is
  // "MotoGP.YYYY.<location>.GP.<session>" — we emit that plus a couple of
  // common variants per session.
  searchTitles(event) {
    const name = event && event.name;
    if (!name) return [];
    const out = new Set();
    ...
    const year = event.date ? event.date.slice(0, 4) : '';
    const sessionLabel = MOTOGP_SESSION_LABEL[session] || '';
    if (session === 'race') {
      if (year) {
        out.add(('MotoGP ' + year + ' ' + loc + ' GP').trim());
        out.add(('MotoGP ' + year + ' ' + loc + ' Grand Prix').trim());
      }
    } else if (sessionLabel) {
      if (year) {
        out.add(('MotoGP ' + year + ' ' + loc + ' GP ' + sessionLabel).trim());
        out.add(('MotoGP ' + year + ' ' + loc + ' ' + sessionLabel).trim());
      }
    }
    return Array.from(out).filter(Boolean);
  },
  ...
  isRelevantStreamTitle(title, event) {
    ...
    const t = title.toLowerCase();
    const loc = motogpLocation(event.name || '').toLowerCase().replace(/\s+/g, '');
    const locStem = loc.replace(/(ese|ian|ish|an|n)$/, '').slice(0, 6);
    const locOk = locStem.length >= 4 && t.replace(/\s+/g, '').includes(locStem);
    if (!locOk) return { ok: false, reason: 'no-location-match' };
    if (!yearMatchesEvent(title, event)) return { ok: false, reason: 'wrong-year' };
  ...
  catalogs: [
    ...
    { id: 'motogp-sprint', name: 'MotoGP Sprint',
      ...
      sort: (a, b) => (b.date || '').localeCompare(a.date || '') },
    { id: 'motogp-practice', name: 'MotoGP Practice',
      filter: (ev) => motogpSession(ev.name) === 'practice' && ev.date && ev.date <= isoToday(),
      sort: (a, b) => (b.date || '').localeCompare(a.date || '') },
  ],

  eventScope: defaultEventScope,

  // Keep race-weekend sessions (Practice/Qualifying/Sprint/Race); drop
...
const all = [ufc, one, wwe, aew, f1, boxing, motogp];
