...
const motogp = {
  ...
  searchTitles(event) {
    ...
    if (!year) return [];
    if (session === 'race') {
      // Sunday race — bare location with no session token.
      out.add(('MotoGP ' + year + ' ' + loc).trim());
    } else if (session === 'sprint') {
      // Saturday sprint — both "Sprint" and "Sprint Race" variants exist
      // across release groups, fire both as separate queries.
      out.add(('MotoGP ' + year + ' ' + loc + ' Sprint').trim());
      out.add(('MotoGP ' + year + ' ' + loc + ' Sprint Race').trim());
    } else if (session === 'qualifying') {
      // Saturday morning. Scene rips split into Qualifying.One / Qualifying.Two
      // — a single "Qualifying" query catches both since Newznab text search
      // is fuzzy.
      out.add(('MotoGP ' + year + ' ' + loc + ' Qualifying').trim());
    }
    return Array.from(out).filter(Boolean);
  },
  ...
  isRelevantStreamTitle(title, event) {
    ...
    if (!t.includes(loc)) return { ok: false, reason: 'no-location-match(' + loc + ')' };
    if (!yearMatchesEvent(title, event)) return { ok: false, reason: 'wrong-year' };
