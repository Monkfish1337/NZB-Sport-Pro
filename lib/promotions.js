...
const wwe = {
  ...
  // mostly just strip colon-prefixed subtitles and append the year for annually-
  // repeating events. Saturday Night's Main Event is date-keyed.
  searchTitles(event) {
    const name = event && event.name;
    if (!name) return [];
    const out = new Set();
    // Strip any subtitle after a colon ("Backlash: France" → "Backlash")
    const bare = name.split(':')[0].trim();
    const year = event.date ? event.date.slice(0, 4) : '';
    // Saturday Night's Main Event — date-keyed, no number
    if (/saturday\s*night.?s?\s*main\s*event/i.test(bare)) {
      if (event.date) {
        out.add('WWE SNME ' + event.date);
        out.add('Saturday Nights Main Event ' + event.date);
        out.add('WWE Saturday Nights Main Event ' + event.date);
      }
      return Array.from(out).filter(Boolean);
    }
