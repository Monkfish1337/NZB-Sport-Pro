...
const ufc = {
  ...
  // Native Newznab gets a small UFC-specific query set. Indexers frequently
  // split a card into "Main Card", "Main Event", and "Prelims" releases;
  // the standard short-handle search can disproportionately return prelims.
  // Keep this separate so UU, Easynews, and torrent discovery retain their
  // established query behaviour.
  nativeNewznabSearchTitles(event) {
    const name = String(event && event.name || '').replace(/\s+/g, ' ').trim();
    if (!name || UFC_CONTENDER_RE.test(name)) return ufc.searchTitles(event);
    const standard = ufc.searchTitles(event);
    const primary = standard[0] || ufc.shortHandle(name);
...
const HARDCODED = [ufc, one, wwe, aew, f1, boxing, motogp, matchOfTheDay, manUnited];
const all = [];
const enabled = [];
const byPrefix = {};
