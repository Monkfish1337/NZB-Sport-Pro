...
// (which uses the addon's auto-generated aliases + persists to streamcache),
...
// and returns the raw results WITHOUT writing to streamcache or applying
...
// Commit picked candidates from the live results into the event's
// streamcache, then optionally warm them on a provider. Uses the addon's
// candidate-cache layer so the next /stream call sees the new rows; the
// warm step adds the magnets to the admin's TB/PM library.
async function commitAndWarm(adminId, eventId, hashes, providerCode, log) {
  ...
  // Merge into existing event cache — keep prior entries (warmer's work),
  ...
  const existing = streamcache.get(eventId) || [];
  ...
  try { streamcache.put(eventId, merged); }
  catch (e) { log && log('  streamcache put failed: ' + e.message); }
  log && log('[power-tool] committed ' + picks.length + ' pick(s) to ' + eventId + ' cache (' + added + ' added, ' + updated + ' updated)');
